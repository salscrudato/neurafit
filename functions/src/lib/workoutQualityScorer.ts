 ...
 */
export function scoreWorkoutQuality(
  workout: WorkoutPlan,
...
function scoreProgramming(
  ...
  exercises.forEach(exercise => {
    ...
    } else if (exerciseName.includes('jump') || exerciseName.includes('sprint') ||
    ...
    if (exercise.restSeconds < minRestSeconds) {
      ...
      score.feedback.push(`${exercise.name}: Rest period too short (${exercise.restSeconds}s, need ${minRestSeconds}s+)`)
...
function scoreSafety(
  ...
  injuries.forEach(injury => {
    ...
    exercises.forEach(exercise => {
      const exerciseName = exercise.name.toLowerCase()
      const hasContraindication = contraindicatedMovements.some(movement => 
        exerciseName.includes(movement)
      )
...
function scoreFeasibility(
  ...
  if (estimatedTime > targetDuration * 1.3) {
    feasibilityScore -= 20
    score.feedback.push(`Estimated time (${estimatedTime}min) significantly exceeds target (${targetDuration}min)`)
  }
  ...
  exercises.forEach(exercise => {
    if (exercise.usesWeight && availableEquipment.length > 0) {
      const needsEquipment = !availableEquipment.some(eq => 
