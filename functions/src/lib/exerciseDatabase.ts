...
export const COMPOUND_EXERCISES: ExerciseTemplate[] = [
  {
    ...
    description: 'Stand with feet shoulder-width apart, toes slightly turned out. Lower your body by pushing hips back and bending knees, keeping chest up and weight on heels. Descend until thighs are parallel to floor. Drive through heels to return to starting position. Breathe in on the way down, out on the way up.',
    formTips: [
  ...
  },
  {
    ...
    description: 'Start in plank position with hands slightly wider than shoulders, body in straight line from head to heels. Lower chest toward floor by bending elbows, keeping them at 45-degree angle to body. Push back up to starting position. Breathe in on the way down, out on the way up.',
    formTips: [
  ...
  },
  {
    ...
    description: 'Stand with feet hip-width apart, holding dumbbells at sides. Hinge at hips by pushing butt back, keeping slight bend in knees. Lower dumbbells along legs until you feel stretch in hamstrings. Drive hips forward to return to standing. Breathe in on the way down, out on the way up.',
    formTips: [
...
export const ISOLATION_EXERCISES: ExerciseTemplate[] = [
  {
    ...
    description: 'Stand with feet hip-width apart, holding dumbbells at sides with palms facing forward. Keeping elbows stationary at sides, curl weights up by contracting biceps. Slowly lower back to starting position with control. Breathe out on the way up, in on the way down.',
    formTips: [
...
export const CARDIO_EXERCISES: ExerciseTemplate[] = [
  {
    ...
    description: 'Stand tall with feet hip-width apart. Lift one knee up toward chest, then quickly switch to lift other knee. Continue alternating at a rapid pace while pumping arms. Land softly on balls of feet. Maintain steady breathing throughout.',
    formTips: [
...
export const MOBILITY_EXERCISES: ExerciseTemplate[] = [
  {
    ...
    difficulty: 'beginner',
    contraindications: [],
    description: 'Stand with feet shoulder-width apart, arms extended out to sides at shoulder height. Make small circles with arms, gradually increasing size. Perform in both forward and backward directions. Keep movements controlled and smooth.',
    formTips: [
  ...
  return allExercises.filter(exercise => {
    ...
    if (userProfile.equipment && userProfile.equipment.length > 0) {
      const hasRequiredEquipment = exercise.equipment.some(eq => 
        userProfile.equipment!.some(userEq => 
          userEq.toLowerCase().includes(eq.toLowerCase()) || 
          eq.toLowerCase().includes(userEq.toLowerCase())
        )
      )
